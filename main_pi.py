...
try:
    import numpy
    # Check if we are running compatible numpy
    if numpy.__version__ >= '2.0.0':
        print(f"[WARN] NumPy version {numpy.__version__} detected.")
        print(" If you experience 'Illegal Instruction' crashes, run ./setup.sh again.")
        # We don't exit here anymore because some custom builds might work.
except ImportError:
    pass
...
class CameraStream:
    ...
    Supports standard Webcams and RPi CSI Cameras.
    """
    def __init__(self, src=0):
        ...
        if platform.system() == 'Windows':
             ...
             # RPi CSI Camera Logic
             # Try GStreamer pipeline first (Best for Libcamera / CSI)
             gstreamer_pipeline = (
             ...
             try:
                 print("[INIT] Attempting to open CSI Camera via GStreamer...")
                 self.stream = cv2.VideoCapture(gstreamer_pipeline, cv2.CAP_GSTREAMER)
                 if not self.stream.isOpened():
                     raise Exception("GStreamer failed")
                 print("[INIT] CSI Camera opened successfully!")
             except Exception as e:
                 print(f"[WARN] CSI Camera GStreamer failed ({e}). Falling back to V4L2 (Index {src})...")
        ...
        if not self.grabbed:
            print("[WARN] Could not read from camera. If using CSI camera, ensure 'Legacy Camera' is enabled or libcamera is working.")
            print("[INFO] Trying to continue anyway (might fail later)...")
            # Don't set stopped=True yet, let the update loop try once more or fail gracefully
